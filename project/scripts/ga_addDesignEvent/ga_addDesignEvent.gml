...
if(argument_count == 1)
{
	if(os_browser == browser_not_a_browser)
	{
		...
		{
			addDesignEvent_mac(argument[0]);
		}
		else if(os_type == os_linux)
		{
			addDesignEvent_linux(argument[0]);
		}
		else if(os_type == os_uwp)
		{
			addDesignEvent_uwp(argument[0]);
		}
	}
	else
	{
...
else if(argument_count == 2)
{
	if(os_browser == browser_not_a_browser)
	{
		...
		{
			addDesignEventWithValue_mac(argument[0], argument[1]);
		}
		else if(os_type == os_linux)
		{
			addDesignEventWithValue_linux(argument[0], argument[1]);
		}
		else if(os_type == os_uwp)
		{
			addDesignEventWithValue_uwp(argument[0], argument[1]);
		}
	}
	else
	{
