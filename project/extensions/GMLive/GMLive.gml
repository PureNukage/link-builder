...
// Generated at 2019-10-18 14:24:23 (14010ms) for v2.2.1+
...
if (live_enabled) {
	...
	switch(l__g[0]){
		...
		case 22:
			...
			ds_list_add(l_r,[52,l__g[1],g_gml_func_script[?l_s],l_n,g_gml_func_args[?l_s],g_gml_func_rest[?l_s],g_gml_inst_data[?l_s],l_out]);
			break;
		case 89:
			l_w=l__g[2];
			l_n=array_length_1d(l_w);
...
if (live_enabled) {
	...
	if(l_argv==""){
		...
		for(var l__g1=l_argc;l_i<l__g1;l_i++){
			...
			var l_tfun;
			if(l_pos>=0){
				if(gml_std_string_pos_ext(string_copy(l_arg,1,l_pos),"?")>=0)l_opt=true;
...
if (live_enabled) {
	...
	if(l_note!=undefined)gml_note_add(l_note);
			...
			if(ds_list_size(l_st)>0){
				...
				switch(l__g1[0]){
					case 53:
						var l_k=l__g1[3];
						var l_d1=l__g1[1];
						if((g_gml_var_flags[?l_s]&2)!=0)gml_std_haxe_enum_tools_set(l_st[|0],[47,l_d1,l_s,l_k]); else return gml_program_error(g_gml_program_seek_inst,"`"+l_s+"` is not an array.",l_d1);
						break;
...
if (live_enabled) {
	...
	var l_name=l_pg[14];
	var l_args1=script_execute(argument[0]);
	var l_th;
	if(ds_map_exists(l_pg[3],l_name)){
		var l_scr=ds_map_find_value(l_pg[3],l_name);
		var l_th1=gml_thread_create(l_pg,l_scr[10],l_args1,array_create(l_scr[7]));
		l_th1[@3]=l_pg[9];
		l_th1[@6]=l_pg[10];
...
if (live_enabled) {
	...
	if(g_live_request_id==undefined){
		...
		if(l_now>g_live_request_time){
			...
			if(g_live_request_guid==undefined){
				g_live_request_id=http_get(g_live_request_url+"/init?password="+g_live_request_password+"&config="+os_get_config()+"&version="+string(104));
			} else {
				...
				if(l_rln>0){
					...
					var l_i2=1;
					for(var l__g12=l_rln;l_i2<l__g12;l_i2++){
						l_url+="+"+room_get_name(l_rl[|l_i2]);
					}
					ds_list_clear(l_rl);
				}
				...
				if(l_rln1>0){
					...
					var l_i3=1;
					for(var l__g13=l_rln1;l_i3<l__g13;l_i3++){
						l_url+="+"+room_get_name(l_rl1[|l_i3]);
					}
					ds_list_clear(l_rl1);
				}
...
if (live_enabled) {
	...
	if(l_map[?"version"]==undefined||l_map[?"version"]<104)show_error("Outdated GMLive server detected! Please update the included files from the extension.",false);
...
if (live_enabled) {
	...
	if(l_ccError==undefined){
		...
		if(ds_map_exists(l_ccProgram[3],l_ccPath)){
			...
			l_th[@3]=l_ccProgram[9];
			l_th[@6]=l_ccProgram[10];
			gml_thread_exec(l_th);
			l_ccThread=l_th;
		...
		case "GMRInstanceLayer":
			if(g_live_room_loader_apply_instances){
				...
				while(++l_i<l_n){
					...
					g_live_room_loader_inst_map_yy[?l_qinst[?"id"]]=l_qinst;
					g_live_room_loader_inst_map_gml[?l_qinst[?"id"]]=l_rnext;
					with (l_rnext) {
...
if (live_enabled) {
	...
	var l__g=0;
	var l__g1=l_rm[?"instanceCreationOrderIDs"];
	while(l__g<array_length_1d(l__g1)){
		var l_id=l__g1[l__g];
		++l__g;
	...
	if(g_live_room_loader_apply_views){
		...
		if(l_qvs!=undefined){
			var l_i=0;
			for(var l__g3=ds_list_size(l_qvs);l_i<l__g3;l_i++){
				var l_qv=l_qvs[|l_i];
		...
		while(l_proc&&l_pos<l_len){
			...
			switch(l_act[0]){
				...
				case 50:
					...
					if(gml_value_is_number(l_v1)){
						...
						if(l_tmp){
							...
							l_len=ds_list_size(l_actions);
							l_stack=l_scope[5];
						} else l_proc=gml_thread_proc_error(l__gthis,("Script index `"+gml_value_print(l_v1)+"` is out of bounds (0..")+string(array_length_1d(gml_std_haxe_boot_wget(this[0],2)))+" excl.)",l_act);
					} else l_proc=gml_thread_proc_error(l__gthis,"Script index must be a number, got `"+gml_value_print(l_v1)+"` ("+gml_value_get_type(l_v1)+")",l_act);
					break;
				case 51:
				...
				case 52:
					...
					l_s="";
					for(l_i=0;l_i<l_n;++l_i){
						if(l_i<l_d)l_s=script_execute(l_argt[l_i],l_w1[l_k+l_i]); else l_s=script_execute(l_act[5],l_w1[l_k+l_i]);
						if(l_s!="")break;
					}
					if(l_i<l_n){
