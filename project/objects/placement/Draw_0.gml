...
if placeable == false {
	...
	var _w = grid_x - floor(size_width/2)
	var _h = grid_y - floor(size_height/2)
	var _x = x+80 - (floor(size_width/2) * cell_width)
	var _y = y+80 - (floor(size_height/2) * cell_height)
	for(var a=0;a<size_width;a++) {
		_h = grid_y - floor(size_height/2)
		_y = y+80 - (floor(size_height/2) * cell_height)
