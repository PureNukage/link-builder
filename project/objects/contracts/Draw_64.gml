...
if button_open == true {
	if ds_list_size(contracts_list) > 0 {
		...
		for(var i=0;i<ds_list_size(contracts_list);i++) {
			...
			draw_set_color(c_dkgray)
			draw_roundrect(buttonX,_yy,buttonX+button_width,_yy+button_height,false)	
			
			//	Draw the Contract outline
			draw_set_color(c_black)
			draw_roundrect(buttonX,_yy,buttonX+button_width,_yy+button_height,true)
			draw_roundrect(buttonX+1,_yy+1,buttonX+button_width-1,_yy+button_height-1,true)
			
			//	Draw the Contract name
			draw_set_color(c_white)
			draw_set_halign(fa_center)
			draw_set_valign(fa_middle)
			draw_text(buttonX+button_width/2,_yy+button_height/2,contract_data[0,0])
